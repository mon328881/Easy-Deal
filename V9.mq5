//+------------------------------------------------------------------+
//|                     太极·夔牛 V9 (新闻日历 + 极简化重构).mq5      |
//|  - 状态识别: ATR + ADX 双指标 (M15)                               |
//|  - 震荡态  : 双向网格 (固定止损 + 篮子止盈, 默认关闭)              |
//|  - 趋势态  : 突破/趋势跟随 + ATR 动态止损                          |
//|  - V9 核心: 引入 MT5 经济日历过滤 (借鉴 GMarket.mq5)               |
//|  作者: Quant Engineer (V8.3 → V9 极简化路线)                       |
//+------------------------------------------------------------------+
#property copyright "太极·夔牛 V9 News-Aware"
...
//==================================================================
// ============================================================
// V9 核心改进 (基于 V8.3 全年 0 笔交易的痛定思痛诊断):
//
// 问题根因: 我们把"过滤器"当成了"安全感", 越加越多 (鞭打+冷启动+H1硬过滤)
//          叠加起来全年 0 笔交易, 等于不存在
//
// V9 解决方案:
//   1. 【引入 MT5 经济日历过滤】(借鉴 GMarket.mq5):
//      - 高重要性新闻(NFP/CPI/FOMC)前后 60 分钟暂停开仓
//      - 这才是黄金真正需要规避的风险时段, 不是"鞭打"
//   2. 【删除鞭打过滤器】(冻结4小时太狠, 用新闻过滤替代)
//   3. 【删除趋势冷启动】(等2根K线毫无意义)
//   4. 【删除每趋势期入场限制】(让市场自己决定)
//   5. 【H1偏好软化】(从硬过滤改为软参考, 日志打印但不阻止)
//   6. 【保留 ADX 状态识别 + 双引擎结构】(V9 的根)
//   7. 【保留出场逻辑修复 V8.2.1】(R:R 改善)
//
// 预期: 全年 60-150 笔, 关键新闻时段自动规避, 趋势期可灵活入场
// ============================================================
...
input double InpBO_BE_TriggerATR   = 0.8;        // ★ V9.1: 1.5→0.8 浮盈 ~2.5 美元即保本
input double InpBO_TrailStartATR   = 1.5;        // ★ V9.1: 2.5→1.5 浮盈 ~5 美元启动追踪
input double InpBO_TrailDistATR    = 0.5;        // ★ V9.1: 0.8→0.5 紧贴市价锁利
...
void UpdateMarketRegime() {
   ...
   // V9 严格二元化: 消除中性带, 任何 ADX 都被划分到 RANGE / TREND_UP / TREND_DOWN
   ...
   if(atrAvg > 0 && atr >= atrAvg * InpATR_HighVolMult) {
      ...
      else             newRegime = REGIME_RANGE;   // 方向不明退回震荡 (而不是 NEUTRAL)
   } else {
      newRegime = REGIME_RANGE;  // V9: ADX 不够强 → 一律视为震荡 (走网格)
   }
   ...
   if(g_regimeCandidateBars >= InpRegimeMinHoldBars && newRegime != g_regime) {
      ...
      // V9: 重置趋势期入场计数器 + 记录入场时间 (兼容字段, 默认放宽)
      g_regimeEnterTime = TimeCurrent();
      g_entriesThisTrend = 0;

      // V9 修复: 状态切换时清空陈旧拒单缓存, 避免诊断板显示旧状态的拒单
      g_lastBlockReason = "";

      // V9 删除: 鞭打过滤器 (改用新闻日历过滤)
...
void OnTick() {
   ...
   // V9: 仅在极端波动时拦截, 中性/未知都按震荡处理 (走网格)
   if(g_regime == REGIME_HIGH_VOL) {
      g_lastBlockReason = StringFormat("当前状态[%s]不开新仓 (ATR > 均值×%.1f)",
                                       RegimeText(g_regime), InpATR_HighVolMult);
      UpdateDashboard();
      PrintDiagnostic(false);
      return;
   }
   ...
   // 7) 分支引擎
   // V9: RANGE/NEUTRAL/UNKNOWN 都走网格 (默认行为, 防御性编程)
   if(g_regime == REGIME_RANGE ||
      g_regime == REGIME_NEUTRAL ||
      g_regime == REGIME_UNKNOWN)            ManageGridEngine();
   if(g_regime == REGIME_TREND_UP ||
